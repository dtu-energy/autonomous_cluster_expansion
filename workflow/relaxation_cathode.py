    ...
    if params['method'] == 'VASP':
        ...
        for a in atom:
            if isinstance(M_ions, str):
                if a.symbol == M_ions:
                    if count < N_redox:
                        a.magmom = get_magmom(M_ions,redox=True)
                        tot_magmom += get_magmom(M_ions,redox=True)
                    else:
                        a.magmom = get_magmom(M_ions,redox=False)
                        tot_magmom += get_magmom(M_ions,redox=False)
                    count += 1
                else:
                    a.magmom = 0
            else:
                if a.symbol in M_ions:
                    if count < N_redox:
                        a.magmom = get_magmom(a.symbol,redox=True)
                        tot_magmom += get_magmom(a.symbol,redox=True)
                    else:
                        a.magmom = get_magmom(a.symbol,redox=False)
                        tot_magmom += get_magmom(a.symbol,redox=False)
                    count += 1
                else:
                    a.magmom = 0
